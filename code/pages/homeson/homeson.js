...
Page({
  ...
  */
  onLoad: function (options) {
    ...
    {
      console.log('key', this.data.info.searchValue);
      this.setData({
    ...
    else if (this.data.info.tg == 2)
    {
      ...
      var ke = this.data.info.searchValue;
      this.setData({
        key: this.data.tagsDict[this.data.info.searchValue],
        stt: "分类标签："
      })
      if (app.globalData.userInfo !== null) {
        this.setData({
      ...
      wx.request({
        url: 'https://group.tttaaabbbccc.club/f/processing/'+ke +'/',
        ...
        success(res) {
          ...
          for (var i = 0; i < res.data.length; i++) {
            ...
            console.log("00000000"+res.data[i])
            if (res.data[i].labels == "") {
              res.data[i]["vie"] = false;
            }
            else {
